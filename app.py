...
class EvaluationRecord(db.Model):
    ...
    fluency_model_a = db.Column(db.Integer)
    fluency_model_b = db.Column(db.Integer)
    sufficient_model_a = db.Column(db.Integer)
    sufficient_model_b = db.Column(db.Integer)
    relevant_model_a = db.Column(db.Integer)
    relevant_model_b = db.Column(db.Integer)
    explanation = db.Column(db.String(1e4))
    completion_b = db.Column(db.String(1e4))
    completion_a_is_acceptable = db.Column(db.Integer)
    completion_b_is_acceptable = db.Column(db.Integer)
...
def signup():
    if request.method == "POST":
        ...
        new_user = User(username=username, password=hashed_password, area="cs_nlp")
...
def get_comparison_results(records, target_model_a, target_model_b):
    ...
    latest_comparison_results = [
        results[-1] for _, results in comparison_results.items()
    ]
    model_wins_counter = Counter(latest_comparison_results)
    model_wins_rates = {
        result: count / len(latest_comparison_results)
...
def summarize_results():
    ...
    model_pairs = set(
        [tuple(sorted([record.model_a, record.model_b])) for record in records]
    )
...
def submit_evaluation():
    ...
    print(COMPARISON_INSTANCES[evaluation_data["index"]].keys())
    # write to the database
    new_record = EvaluationRecord(
        instance_index=evaluation_data["index"],
        instance_id=COMPARISON_INSTANCES[evaluation_data["index"]]["instance_id"],
        ...
        fluency_model_a=evaluation_data["fluency_model_a"],
        fluency_model_b=evaluation_data["fluency_model_b"],
        sufficient_model_a=evaluation_data["sufficient_model_a"],
        sufficient_model_b=evaluation_data["sufficient_model_b"],
        relevant_model_a=evaluation_data["relevant_model_a"],
        relevant_model_b=evaluation_data["relevant_model_b"],
...
def submit_feedback():
    ...
    new_record = EvaluationRecord(
        ...
        instance_id=COMPARISON_INSTANCES[feedback_data["index"]]["instance_id"],
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=5001, help="The port of the server."
    )
    parser.add_argument(
        "--debug", action="store_true", help="Whether to run the server in debug mode."
    )
    parser.add_argument("--db_path", type=str, default="evaluation.db")
    args = parser.parse_args()

    if not os.path.exists(os.path.join(os.getcwd(), "data", args.db_path)):
        print(os.path.join(os.getcwd(), "data", args.db_path))
